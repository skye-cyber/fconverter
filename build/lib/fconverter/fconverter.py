...
def word_to_pdf(word_file, pdf_file):
    try:
        if os.name == 'posix':  # Check if running on Linux
            ...
            doc.save(pdf_file)
        logger.info(f"\033[1;95m Successfully converted {word_file} to {pdf_file}\033[0m")
    except Exception as e:
        logger.info(f"Error converting {word_file} to {pdf_file}: {e}")
...
def word_to_ppt(word_file, ppt_file):
    ...
    try:
        ...
        for paragraph in document.paragraphs:
            ...
            logger.info(f"\033[1;95mSuccessfully converted {word_file} to {ppt_file}\033[0m")
    except Exception as e:
        logger.error(f"Error converting {word_file} to {ppt_file}: {e}")
...
def word_to_txt(word_file, txt_file):
    try:
        ...
        with open(txt_file, 'w', encoding='utf-8') as f:
            ...
            logger.info(f"\033[1;95mSuccessfully converted {word_file} to {txt_file}\033[0m")
    except Exception as e:
        logger.error(f"Error converting {word_file} to {txt_file}: {e}")
        with open("conversion.log", "a") as log_file:
...
def pdf_to_txt(pdf_file, txt_file):
    try:
        ...
        with open(txt_file, 'w', encoding='utf-8') as f:
            f.write(text)
            logger.info(f"\033[1;95mSuccessfully converted {pdf_file} to {txt_file}\033[0m")
    except Exception as e:
        logger.error(f"Error converting {pdf_file} to {txt_file}: {e}")
        with open("conversion.log", "a") as log_file:
...
def ppt_to_word(ppt_file, word_file):
    try:
        ...
        logger.info(f"\033[1;95mSuccessfully converted {ppt_file} to {word_file}\033[0m")
    except Exception as e:
        logger.error(f"Error converting {ppt_file} to {word_file}:\n>>> {e}")
        with open("conversion.log", "a") as log_file:
...
def text_to_word(text_file, word_file):
    try:
        ...
        doc.save(word_file)
        logger.info(f"\033[1;95mSuccessfully converted {text_file} to {word_file}\033[0m")
...
def main():
    ...
    parser.add_argument('conversion_type', type=int, help='''The type of
                        ...
                        \033[1;96m 5:\033[0m PDF to TXT,  \033[1;96m 6:\033[0mPPTX to Word,\
                         ...
                         \033[1;96m 11:\033[0m XLSX to TXT \033[1;96m 12:\033[0m Image Text Extraction,\
                          \033[1;96m 13:\033[0m Convert xlsx to sqlite db  \033[1;96m 14:\033[0m XLSX to CSV\
                          \033[1;96m 15:\033[0m Other conversions including the above\n\n''')
    '''\033[38;5;226;5m Note that sometimes you might need to navigate
    to the directory where the file to be converted is located.\033[0m'''
    ...
    if args.conversion_type == 1:
        ...
        logger.error("\033[1;91m Invalid conversion type. Please enter a number from 1 to 14.\033[0m")
